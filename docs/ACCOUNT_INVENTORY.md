...
Catalog version `2026.08.5` materially expands reviewed German, European, and global coverage across mailbox providers; finance and payments; password, security, and identity services; cloud, developer, and work platforms; commerce and travel; communications; gaming; government, health, and insurance; publishers; and clearly lower-impact consumer services. Gaming accounts are `Critical` when they can hold purchases, balance, tradable or valuable digital inventory, marketplace access, or material social identity; a generic `games` hint still proves nothing. The [broad August 2026 review](ACCOUNT_CLASSIFICATION_REVIEW_2026_08.md) and its [NonCritical follow-up](ACCOUNT_CLASSIFICATION_NONCRITICAL_REVIEW_2026_08.md) record the candidate material, review method, additions, evidence, and representative exclusions. Those reports are audit artifacts and are not loaded at runtime.

The repository coverage goal is at least **1,000 canonical provider/account realms in each category**. It is a planning and coverage target, not authority to copy a third-party category into runtime or to split one provider family into artificial records. Version `2026.08.5` reaches the `NonCritical` goal; `Email` and `Critical` remain below the goal and require separate reviewed expansions. If unpwn cannot make a defensible classification from a concrete provider alias or domain, the result stays `Unknown` and the user reviews it during triage.
