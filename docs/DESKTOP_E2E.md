...
| browser start, navigation context, close, cleanup, orphaned cleanup retry | `golden`; `reviewed-browser-step-hierarchy`; `browser-close-preserves-recovery`; browser lifecycle/security tests | Forced crash during native-profile teardown remains an extended desktop candidate; lifecycle tests inject the failure and verify conservative restart handling. |
