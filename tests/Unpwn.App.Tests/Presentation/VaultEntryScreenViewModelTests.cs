...
public sealed class VaultEntryScreenViewModelTests
{
    ...
    public async Task ReturningUserGetsNewestExistingVaultAsPrimaryAction()
    {
        ...
        Assert.DoesNotContain(
            Path.GetDirectoryName(newest)!,
            viewModel.PrimaryVaultPathContext,
    ...
    [Fact]
    public async Task StaleRecentVaultIsNotOfferedOrRecreatedAtStalePath()
    {
    ...
    {
        const string password = "synthetic-long-vault-password";
        var lifecycle = new TestVaultLifecycleService();
        var wizard = new RecoveryWizardSessionService(DateTimeOffset.UnixEpoch);
        var viewModel = CreateViewModel(lifecycle, wizard);
    ...
    {
        const string password = "synthetic-existing-vault-password";
    ...
    {
        const string password = "synthetic-locked-vault-password";
    ...
    {
        const string vaultPassword = "synthetic-long-vault-password";
        var lifecycle = new TestVaultLifecycleService();
        var viewModel = CreateViewModel(
            lifecycle,
            new RecoveryWizardSessionService(DateTimeOffset.UnixEpoch));
        ...
        viewModel.CurrentPassword = "synthetic-current-vault-password";
        viewModel.NewPassword = "synthetic-new-vault-password";
        viewModel.ConfirmNewPassword = "synthetic-new-vault-password";
    ...
    {
        const string password = "synthetic-wrong-vault-password";
        var lifecycle = new TestVaultLifecycleService
        {
    ...
    private sealed class TestVaultPathProvider(
        string? defaultPath = null,
        IEnumerable<string>? existingPaths = null) : IVaultPathProvider
    {
        ...
        public string DefaultPath { get; } =
            defaultPath ?? Path.GetFullPath("synthetic-default-vault.db");
    ...
    private sealed class TestVaultLifecycleService : IVaultLifecycleService
    {
            ...
            CancellationToken cancellationToken)
        {
            RecentVaults =
            [
                .. RecentVaults.Where(reference => !string.Equals(
                    reference.Path,
                    path,
                    OperatingSystem.IsWindows()
