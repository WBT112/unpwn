...
public sealed class SyntheticRecoveryProviderTests
{
    ...
    {
        await using var provider = await SyntheticRecoveryProvider.StartAsync();
    ...
    [Fact]
    public async Task PasswordChangeScenarioExposesManagedBrowserContractEvidence()
    {
        await using var provider = await SyntheticRecoveryProvider.StartAsync();
        using HttpClient client = new() { BaseAddress = provider.BaseAddress };

        string body = await client.GetStringAsync("/settings/password?scenario=password-change");

        Assert.Contains("data-unpwn-provider=\"synthetic\"", body, StringComparison.Ordinal);
        Assert.Contains("data-unpwn-workflow=\"password-change\"", body, StringComparison.Ordinal);
        Assert.Contains("data-testid=\"new-password\"", body, StringComparison.Ordinal);
        Assert.Contains("data-testid=\"confirm-password\"", body, StringComparison.Ordinal);
    }

    [Theory]
    [InlineData("/mfa?scenario=mfa-pause", "mfa")]
    [InlineData("/captcha?scenario=captcha-pause", "captcha")]
    [InlineData("/reset-link?scenario=email-link-handoff", "email-link")]
    public async Task StopScenariosExposeManagedBrowserPauseEvidence(string path, string stopReason)
    {
        await using var provider = await SyntheticRecoveryProvider.StartAsync();
        using HttpClient client = new() { BaseAddress = provider.BaseAddress };
        ...
        Assert.Contains($"data-unpwn-stop-reason=\"{stopReason}\"", body, StringComparison.Ordinal);
    }
}
